...
function ComponentPlot:new(win_plot, config)
  return setmetatable({
    ...
    horizontal_border_win_plot = nil,
...
function ComponentPlot:configure_bounds()
  ...
  local has_horizontal_border = config.horizontal_horder
  ...
  if win_plot.row + win_plot.height > global_height then
    if self.is_at_cursor then
      ...
      if has_horizontal_border then
        win_plot.row = win_plot.row - horizontal_border_height
      end
    else
...
function ComponentPlot:configure_height()
  ...
  local has_horizontal_border = config.horizontal_border
  local has_line_number = config.line_number
  ...
  if has_horizontal_border then
    win_plot.height = win_plot.height - horizontal_border_height
  end
  if has_line_number then
    self.line_number_win_plot.height = win_plot.height
  end
...
function ComponentPlot:configure_row()
  ...
  local has_line_number = config.line_number
  local header_height = HeaderElement:get_height()
  local horizontal_border_win_plot = self.horizontal_border_win_plot
  ...
  if has_horizontal_border then
    horizontal_border_win_plot.row = win_plot.row
    horizontal_border_win_plot.row = horizontal_border_win_plot.row
      + win_plot.height
  end
  return self
end
...
function ComponentPlot:build()
  ...
  self.horizontal_border_win_plot = utils.object.clone(win_plot)
